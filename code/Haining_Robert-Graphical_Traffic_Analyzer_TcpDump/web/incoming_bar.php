<?php
	/*Incoming Traffic Bar Graph, based on ports
	...
	*/
	...
	$sql_que = "SELECT $port_type FROM $mysql_table "
			. "WHERE ($port_type > 1023) AND (dst_ip = \"$my_ip\")";
	if( ($mysql_result = mysql_query($sql_que)) == FALSE) {
		print "Query failed : " . mysql_error();
		exit();
	}
	...
	$graph->title->set("Number of Incoming Packets over the Last 24 Hours");   
	$subtitle = "generated at " . date("g:iA") . " on " . date("F d, Y");
	$graph->subtitle->Set($subtitle);//subtitle

	$graph->Stroke("/var/www/html/incoming_bar.png");//draws graph & places it in this file
