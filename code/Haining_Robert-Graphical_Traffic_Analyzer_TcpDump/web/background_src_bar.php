<?php
	/*Background Traffic Bar Graph, based on ports
	...
	*/
	...
	$sql_que = "SELECT protocol, $port_type FROM $mysql_table "
			...
			. "AND (dst_ip != \"$my_ip\") AND (src_ip != \"$my_ip\") "
	...
	$graph->title->set("Background Traffic From Ports over the Last 24 Hours");   
	$subtitle = "generated at " . date("g:iA") . " on " . date("F d, Y");
	$graph->subtitle->Set($subtitle);//subtitle

	$graph->Stroke("/var/www/html/background_src_bar.png");//draws graph & places it in this file
